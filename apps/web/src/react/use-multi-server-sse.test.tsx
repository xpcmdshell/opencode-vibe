...
describe("useMultiServerSSE", () => {
  ...
  test("updates store with session status via handleEvent", async () => {
    ...
    await waitFor(() => {
      const state = useOpencodeStore.getState();
      expect(
        state.directories["/test/project"]?.sessionStatus["session-123"],
      ).toBe("running");
    });
  });
  ...
  test("handles multiple events for different sessions", async () => {
    ...
    await waitFor(() => {
      const state = useOpencodeStore.getState();
      expect(
        state.directories["/test/project"]?.sessionStatus["session-1"],
      ).toBe("running");
      expect(
        state.directories["/test/project"]?.sessionStatus["session-2"],
      ).toBe("completed");
    });
  });
  ...
  test("handles events across multiple directories", async () => {
    ...
    await waitFor(() => {
      const state = useOpencodeStore.getState();
      expect(state.directories["/project-a"]?.sessionStatus["session-a"]).toBe(
        "running",
      );
      expect(state.directories["/project-b"]?.sessionStatus["session-b"]).toBe(
        "completed",
      );
    });
  });
